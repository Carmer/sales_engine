...
class InvoiceItemRepositortyTest < Minitest::Test

  def setup
    @invoice_item_objects = SalesEngine.new.invoice_item_repository
  end
  ...
  def test_it_can_inspec_itself
    assert_equal "#<InvoiceItemRepository 101 rows>", @invoice_item_objects.inspect
